...
namespace karto
{
  ...
  public:
    ...
    void Clear()
    {
      ...
      {
        // delete each vertex
		  forEachMe(typename std::vector<Vertex<T>*>, &(indexIter->second))
        {
          delete *iter;
        }
      }
  ...
  private:
    ...
    public: // changed by myself, delete this line later.
    /**
    ...
    private: // changed by myself, delete this line later.
    /**
  ...
  public:
    /**
     * Create a correlation grid of given size and parameters  创建一个相关性网格
     * @param width                 宽度
     * @param height                高度
     * @param resolution            分辨率
     * @param smearDeviation        平滑参数
     * @return correlation grid     返回值
     */
    ...
    /**
     * Gets the index into the data pointer of the given grid coordinate 在给定的网格坐标系中得到指定网格的索引值，这里需要考虑边界的问题
     * @param rGrid
     * @param boundaryCheck
     * @return grid index
     */
    ...
    /**
     * Get the Region Of Interest (ROI)  返回感兴趣区域
    ...
    /**
     * Sets the Region Of Interest (ROI) 设置感兴趣区域
     * @param roi
     */
    ...
    /**
     * Smear cell if the cell at the given point is marked as "occupied" 如果一个网格点标记为 occupied，则对其进行 smear，根据高斯分布，给周围的点也赋予一定的权重
     * @param rGridPoint
     */
    inline void SmearPoint(const Vector2<kt_int32s>& rGridPoint)
    {
	  ...
	  if (GetDataPointer()[gridIndex] != GridStates_Occupied) /* 必须为 occupied 网格,否则直接返回 */
      {
        return;
      }
      ...
      for (kt_int32s j = -halfKernel; j <= halfKernel; j++)
      {
        ...
        for (kt_int32s i = -halfKernel; i <= halfKernel; i++)
        {
		  ...
		  /* 理解其中的物理意义：如果相邻网格的值大于本网格的值，则直接赋值相邻网格的值，注意在CalculateKernel()函数中查看 m_pKernel 构造方法*/
  ...
  protected:
    /**
     * Constructs a correlation grid of given size and parameters  构建一个相关性网格
     * @param width
     * @param height
     * @param borderSize
     * @param resolution
    ...
    CorrelationGrid(kt_int32u width, kt_int32u height, kt_int32u borderSize,kt_double resolution, kt_double smearDeviation)
      : Grid<kt_int8u>(width + borderSize * 2, height + borderSize * 2)  /* 这里的 grid 是进行了边缘扩展过的 */
      , m_SmearDeviation(smearDeviation)
      , m_pKernel(NULL)
    {
      GetCoordinateConverter()->SetScale(1.0 / resolution);

      // setup region of interest 设置感兴趣区域
      m_Roi = Rectangle2<kt_int32s>(borderSize, borderSize, width, height);

      // calculate kernel, 填充了 m_pKernel 中的数据
      CalculateKernel();
    }

    /**
     * Sets up the kernel for grid smearing. 建立 grid smear 的核
     */
    virtual void CalculateKernel()
    {
      ...
      for (kt_int32s i = -halfKernel; i <= halfKernel; i++)
      {
        ...
        {
#ifdef WIN32
          kt_double distanceFromMean = _hypot(i * resolution, j * resolution); // 相当于计算距离
#else
          kt_double distanceFromMean = hypot(i * resolution, j * resolution);
#endif
          kt_double z = exp(-0.5 * pow(distanceFromMean / m_SmearDeviation, 2));   // 其实是二维高斯概率分布
		  // 如果中心为占用，那么相邻的为占中的概率
     ...
     * @param smearDeviation 相当于两倍的标准差，
     * @param resolution
     * @return kernel half-size based on the parameters 计算核的大小,物理意义不是很清楚
     */
  ...
  public:
     ...
     * @return strength of response
     */
    kt_double CorrelateScan(LocalizedRangeScan* pScan,  /* 当前的激光扫描 */
                            const Pose2& rSearchCenter, /* 搜索中心       */
                            const Vector2<kt_double>& rSearchSpaceOffset,     /* 相对于搜索中心的偏移量 */
                            const Vector2<kt_double>& rSearchSpaceResolution, /* 搜索中心分辨率         */
                            kt_double searchAngleOffset,
                            kt_double searchAngleResolution,
                            kt_bool doPenalize, /* 是否惩罚远离搜索中心的位置 */
                            Pose2& rMean,       /* 输出的最佳位置信息 */
                            Matrix3& rCovariance, /* 输出的最佳位置协方差 */
                            kt_bool doingFineMatch); /* 是否做精细搜索 */
  ...
  /**
   * Manages the devices for the mapper 管理传感器设备设备
   */
  ...
  public:
    /**
     * Constructor  构造函数， runningBufferMaximumSize 缓冲区的最大值，runningBufferMaximumDistance 距离的最大值
     */
  ...
  public:
    /**
     * Registers a sensor (with given name); do  注册传感器
    ...
    /**
     * Gets scan from given sensor with given ID  根据传感器名称和ID号获取扫描数据
     * @param rSensorName
     * @param scanIndex
    ...
    /**
     * Gets names of all sensors 获取所有传感器的名称
     * @return sensor names
     */
    ...
    /**
     * Gets last scan of given sensor 获取给定传感器名称的最新数据
     * @param rSensorName
    ...
    /**
     * Sets the last scan of device of given scan 设置给定传感器名称的最新数据
     * @param pScan
     */
    inline void SetLastScan(LocalizedRangeScan* pScan);

    /**
     * Gets the scan with the given unique id  获取指定 ID 号的传感器数据
     * @param id
     * @return scan
     */
  ...
  private:
     ...
     * @param rSensorName  根据传感器的名字查找对应的传感器
     * @return ScanManager
     */
  ...
  private:
    ...
    kt_int32s m_NextScanId; /* scan 数据的编号 */
  ...
  private:
    ...
    /**
     * The size of the search grid used by the matcher. 搜索网格的大小
     * Default value is 0.3 meters which tells the matcher to use a 30cm x 30cm grid.
     */
    ...
    /**
     * The resolution (size of a grid cell) of the correlation grid.
     * Default value is 0.01 meters. 相关性网格的分辨率
     */
    Parameter<kt_double>* m_pCorrelationSearchSpaceResolution;

    /**
     * The point readings are smeared by this value in X and Y to create a smoother response.
     * Default value is 0.03 meters. 平滑相应的空间
     */
